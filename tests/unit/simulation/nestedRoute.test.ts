...
describe("D101: two-layer rocks cost no pilot a belt", () => {
   ...
   */
  it("D101: no pilot gains a stall at Neptune or Pluto", () => {
    for (const [name] of PILOTS) {
      for (const stop of NESTING_STOPS) {
        ...
        expect(
          on.stalls,
          `${name} stalled ${on.stalls} times at ${stop} with two-layer rocks, ` +
            `against ${off.stalls} without, over ${on.belts} belts`,
        ).toBeLessThanOrEqual(off.stalls);
      }
    }
  });

  it("D101: and the baseline it is measured against is still zero", () => {
    // The claim above is a comparison, and a comparison against a number nobody
    // states is how a regression hides inside a "no worse than" assertion.
    for (const [name] of PILOTS) {
      expect(without[name]!.stalls, `${name} without nesting`).toBe(0);
      expect(withNesting[name]!.stalls, `${name} with nesting`).toBe(0);
    }
  });

  it("D101: no pilot gains a stall ANYWHERE on the route either", () => {
    // Nesting only fires at two stops, but it changes the controller's inputs
    // there and the knob travels onward - so a belt lost at Pluto is not the
    // only way this could cost somebody a run.
    for (const [name] of PILOTS) {
      for (let i = 0; i < BELT_STOP_IDS.length; i += 1) {
        const on = withNesting[name]!.stops[i] as StopRow;
        const off = without[name]!.stops[i] as StopRow;
        expect(on.stalls, `${name} gained stalls at ${on.stop}`).toBeLessThanOrEqual(
          off.stalls,
        );
      }
    }
