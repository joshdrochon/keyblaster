...
- AC-11.2 A returning profile is not asked to run the full first-run ritual: it gets D99's launch ceremony instead, which blends into the stored baseline rather than replacing it and runs on shorter beats. Since UR-57 the ceremony asks for the same 5-6 words, so D51's "once per profile" survives only as the difference between measuring and re-measuring, not as a cost (collision C15). → U + E.
- AC-11.3 Feels like narrative, not a test: no score, no accuracy shown during it. → V.
- AC-11.4 Every stop after the first prompts the player to type: the launch ceremony plans words on all three of D81's steps, each honouring its own declared `minWords`/`maxWords`, from the stop's own pool; the Pre-flight screen is never mounted with nothing to type unless the pool cannot supply the words (D99, UR-57). → U + E.
...
- AC-21.9 Shadow's clips are rendered from ONE pinned model, stability and voice (D102): `eleven_flash_v2_5`, `stability 0.92`, voice `J1UkN5Wmr20DZIiLKXHI`. Both the render script's defaults and the shipped manifest record it, and every clip in the manifest came from that voice — the set was re-rendered twice by accident and the originals are gone, so a default nobody used is not enough. → U (`tests/unit/audio/voiceModel.test.ts`).
...
- AC-21.3 Every event (lock, keystroke, typo, blast, hit, shield, warp charge, warp, beacon, UI nav, combo milestone) has ≥ 3 SFX variants; consecutive plays never repeat a variant. → U.
