...
import {
  emitLiveTrophies,
  resetLiveTrophies,
} from "@game/ui/trophyToastLayout.js";
...
export class FlightScene extends Phaser.Scene {
  ...
  init(data: Partial<FlightConfig>): void {
    ...
    this.bestCombo = 0;
    // A fresh attempt announces its streak trophies again from zero.
    resetLiveTrophies(this.game);
  ...
  create(): void {
    ...
    // `this.time.now` is set from `game.loop.time` in the scene clock's
    // preUpdate, which has NOT run yet at create(). On the first belt of a page
    // load it happens to read 0; on every belt after it holds the PREVIOUS
    // belt's last frame time, so `elapsedMs` silently measures the warp, the
    // beacon, the stage report, the map and the briefing as well as this belt.
    // Measured: a second-stop report showed 4 wpm where the child typed ~30,
    // and `liveCap` read the ramp as already finished at second zero, so the
    // board opened at full depth. Same timeline, just a value that is current.
    this.stageStartMs = this.game.loop.time;
  ...
  private spawnRock(
    ...
    const fallMs = fallTimeMs({
      ...
      //
      // D101: AND THE DEPTH IS COUNTED IN WORDS, NOT IN ROCK OBJECTS. The
      // typist is one server (AC-2.1) and a two-layer rock occupies them for
      // TWO words, so a rock spawning behind an intact shell has a queue one
      // longer than `this.rocks.length` says. Counted as rocks, the shell's
      // second word was free to everything behind it: the budget was sized for
      // a queue half as long as the one actually ahead, and the rock behind it
      // spent 56-86% of its fall waiting for the typist rather than being
      // typed. `liveWords()` is `@engine/nested.liveWordsOf` - a cracked rock
      // counts 1 again the instant its shell comes off - so this and AC-2.1's
      // first-letter rule cannot disagree about what is live. On a board with
      // no shell on it this is `this.rocks.length` exactly, so nothing changes
      // for anybody who is not queued behind a two-word rock.
      liveCount: this.liveWords().length,
    });
    ...
    const coreFallMs =
      core === null
        ...
        : fallTimeMs({
            ...
            knobs: this.controller.knobs,
            spread: draws.spread,
            // UR-84 / C22: the core is budgeted as the rock it would have been,
            // which means AT THIS STOP and ON THIS BOARD. Omitting these two is
            // not neutral - it hands the inner word of a Pluto rock the Mars,
            // empty-board budget, so the slowest object on the screen is the
            // one at the hardest stop. The shell one call up passes both.
            stop: this.cfg.stopId,
            // In WORDS, exactly as the shell one call up. The two must stay
            // identical or the pair's budget is two different boards' budgets
            // added together.
            liveCount: this.liveWords().length,
          });
  ...
  private onBlast(
    ...
    // D101/C24: a streak trophy is the only kind answerable mid-belt,
    // so it is the only kind announced here. The third argument is what
    // the child already holds, so a re-earned one stays silent.
    emitLiveTrophies(this.game, this.combo.combo, activeProfile(this)?.trophies ?? []);
