...
const HANG: ArmPose = { angleDeg: 12, reach: 0.82, lift: 0.5 };
...
const POSES: Readonly<Record<ShadowPose, PoseSpec>> = {
  ...
  pointing: {
    eyes: "arcHappy",
    leftArm: HANG,
    rightArm: { angleDeg: 128, reach: 0.9, lift: 0.1 },
    tiltDeg: -4,
    sink: -0.02,
    hover: 1.05,
    sparks: 0,
  ...
  cheering: {
    eyes: "arcHappy",
    leftArm: { angleDeg: 140, reach: 0.88, lift: 0.0 },
    rightArm: { angleDeg: 140, reach: 0.88, lift: 0.0 },
    tiltDeg: 0,
    sink: -0.08,
    hover: 1.25,
    sparks: 6,
  ...
  worried: {
    ...
    eyes: "small",
    leftArm: { angleDeg: -30, reach: 0.5, lift: 0.55 },
    rightArm: { angleDeg: -30, reach: 0.5, lift: 0.55 },
    tiltDeg: 3,
    sink: 0.06,
    hover: 0.8,
    sparks: 0,
  ...
  asleep: {
    eyes: "arcClosed",
    leftArm: { angleDeg: 16, reach: 0.78, lift: 0.58 },
    rightArm: { angleDeg: 16, reach: 0.78, lift: 0.58 },
    tiltDeg: -7,
    sink: 0.16,
    hover: 0.55,
    sparks: 0,
...
function drawEarPod(
  ...
  const x = side * 0.9 * R;
  const y = spec.sink * R - 0.1 * R;
  g.fillStyle(SHADOW_COLORS.bodyLow, 1);
  g.fillEllipse(x, y, 0.56 * R, 0.74 * R);
  g.fillStyle(SHADOW_COLORS.body, 1);
  g.fillEllipse(x - side * 0.03 * R, y - 0.03 * R, 0.46 * R, 0.62 * R);
  ...
  g.strokeCircle(x, y, 0.16 * R);
  g.fillStyle(SHADOW_COLORS.port, 0.3);
  g.fillCircle(x, y, 0.13 * R);
}
...
function drawBody(g: Phaser.GameObjects.Graphics, spec: PoseSpec): void {
  ...
  g.fillStyle(SHADOW_COLORS.specular, 0.4);
...
function eyeGlow(g: Phaser.GameObjects.Graphics, cx: number, cy: number, r: number): void {
  g.fillStyle(SHADOW_COLORS.glow, 0.2);
...
function drawEyes(g: Phaser.GameObjects.Graphics, spec: PoseSpec): void {
  ...
  const arcUp = (cx: number, w: number): void => {
    ...
    eyeGlow(g, cx, cy, w * 0.9);
    g.lineStyle(stroke, SHADOW_COLORS.glowBright, 1);
    g.beginPath();
    g.arc(cx, cy + 0.06 * R, w * 0.55, 196 * DEG, 344 * DEG, false);
    g.strokePath();
  };
  ...
  const arcDown = (cx: number, w: number): void => {
    ...
    g.beginPath();
    g.arc(cx, cy - 0.08 * R, w * 0.55, 20 * DEG, 160 * DEG, false);
    g.strokePath();
  };

  switch (spec.eyes) {
    case "capsule":
      capsule(-EYE_DX, 0.44 * R, 0.2 * R);
      capsule(EYE_DX, 0.44 * R, 0.2 * R);
      break;
    case "arcHappy":
      arcUp(-EYE_DX, 0.46 * R);
      ...
      break;
    case "arcClosed":
      arcDown(-EYE_DX, 0.46 * R);
      arcDown(EYE_DX, 0.46 * R);
      break;
    case "small":
      capsule(-EYE_DX * 0.94, 0.24 * R, 0.18 * R);
      capsule(EYE_DX * 0.94, 0.24 * R, 0.18 * R);
      break;
    case "wink":
      capsule(-EYE_DX, 0.44 * R, 0.2 * R);
...
export function drawShadow(
  ...
  root.add([hover, back, bodyG, rimGlow, plateG, eyesG, frontG]);
  ...
  const render = (next: ShadowPose): void => {
    ...
    for (const g of [hover, back, bodyG, rimGlow, plateG, eyesG, frontG]) g.clear();

    drawHover(hover, spec);

    drawArm(back, spec.leftArm, -1);
    drawArm(back, spec.rightArm, 1);
  ...
  const figure: ShadowFigure = {
    ...
    update(timeMs: number) {
      ...
      for (const g of [back, bodyG, rimGlow, plateG, eyesG, frontG]) g.setY(lift);
    },
    destroy() {
      root.destroy(true);
    },
