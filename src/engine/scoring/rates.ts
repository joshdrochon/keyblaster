 ...
 * delta, average and render. Non-positive elapsed time is treated the same way
 * (a clock that went backwards is not evidence of infinite speed).
 */
...
export function accuracy(hits: number, typos: number): number {
  const attempts = Math.max(0, hits) + Math.max(0, typos);
  if (attempts <= 0) return 1;
  return Math.max(0, hits) / attempts;
}
